...
def fetch_data():
    try:
        ...
        # Fallback sample data if database is empty or starting up
        return pd.DataFrame({
    ...
    html.Div(style={'display': 'flex', 'justifyContent': 'center', 'gap': '20px', 'marginBottom': '20px'}, children=[
        html.Div([
            ...
            dcc.Dropdown(
                id='metric-dropdown',
                options=[
                    {'label': 'Water Temperature (°C)', 'value': 'water_temperature_c'},
...
# Interactive Callbacks to update charts dynamically
@callback(
...
def update_dashboard(selected_metric):
    ...
    # Scatter Mapbox / Geographic scatter plot
    ...
    # Bar chart breakdown by station
    fig_bar = px.bar(
        df, 
        x='station_id', 
        y=selected_metric, 
